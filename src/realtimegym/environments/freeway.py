...
from .render.freeway_render import FreewayRender
...
class FreewayEnv(BaseEnv):
    ...
    def step(self, action):
        ...
        for car in self.cars:
            ...
            if car[0] < 0:
                ...
                if abs(car[3]) >= 1:
                    ...
                    car[0] += int(1 / car[3])
            # collision check
            for length_offset in range(car[4]):
                if car[0] + length_offset * dir == 4 and self.pos == car[1]:
                    self.pos = 9
        ...
        if not self.terminal and self._just_reset:
            R = self.reward
            G = self.game_turn
            self.random = np.random.RandomState(self.seed)
            # Call internal reset without returning
    ...
    def _internal_reset(self):
        """Internal reset helper to avoid returning values."""
        self.chosen_freeways = self.random.choice(range(0, 8), 8, replace=False)
        self.chosen = [True if i in self.chosen_freeways else False for i in range(8)]
        self._randomize_cars()
        self.pos = 9
    ...
    def summary(self):
        if self.terminal:
            ...
            print(
                f"Seed {self.seed} position: {9 - self.pos}, turn: {self.game_turn}, reward: {self.reward}"
            )
