...
from environment.overcooked_new.src.overcooked_ai_py.utils import pos_distance, read_layout_dict, classproperty
from environment.overcooked_new.src.overcooked_ai_py.mdp.actions import Action, Direction
import copy
...
class SoupState(ObjectState):
    ...
    @property
    def is_full(self):
        # print(self.is_idle, self.ingredients)
...
class OvercookedState(object):
    ...
    def __init__(self, players, objects, bonus_orders=[], all_orders=[], timestep=0, put_first_tomato=[False, False], **kwargs):
        """
        ...
        self.put_first_tomato = put_first_tomato
        assert (not (timestep == 0 and any(put_first_tomato))), (put_first_tomato)
    ...
    @classmethod
    def from_players_pos_and_or(cls, players_pos_and_or, bonus_orders=[], all_orders=[], put_first_tomato=[False, False]):
        """
        Make a dummy OvercookedState with no objects based on the passed in player
        positions and orientations and order list
        """
        # print("from_players_pos_and_or")
        return cls(
            [PlayerState(*player_pos_and_or) for player_pos_and_or in players_pos_and_or], 
            objects={}, bonus_orders=bonus_orders, all_orders=all_orders, put_first_tomato=put_first_tomato)

    @classmethod
    def from_player_positions(cls, player_positions, bonus_orders=[], all_orders=[], put_first_tomato=[False, False]):
        """
        ...
        return cls.from_players_pos_and_or(dummy_pos_and_or, bonus_orders, all_orders, put_first_tomato)
    ...
    def deepcopy(self):
        return OvercookedState(
            ...
            put_first_tomato=copy.deepcopy(self.put_first_tomato),)
    ...
    def all_order_info(self):
        return [order.to_dict2() for order in self.all_orders]
...
class OvercookedGridworld(object):
    ...
    def get_standard_start_state(self):
        ...
        assert (not any(start_state.put_first_tomato)), (start_state.put_first_tomato, start_state.timestep)
        return start_state
    ...
    def get_state_transition(self, state, joint_action, display_phi=False, motion_planner=None):
        ...
        sparse_reward_by_agent, shaped_reward_by_agent, shaped_info_by_agent = self.resolve_interacts(new_state, joint_action, events_infos)
    ...
    def can_begin_cook_soup(self, state, player_idx):
    ...
    def resolve_interacts(self, new_state, joint_action, events_infos):
        ...
        for player_idx, (player, action) in enumerate(zip(new_state.players, joint_action)):
            ...
            USEFUL_TOMATO_PICKUP = self.useful_tomato_pickup(new_state, player_idx)
            ...
            if terrain_type == 'X':
                ...
                if player.has_object() and not new_state.has_object(i_pos):
                    ...
                    # if obj_name == "tomato" and USEFUL_TOMATO_PICKUP:
                    #     shaped_info[player_idx]["useful_tomato_pickup"] += 1
                    #     shaped_reward[player_idx] += 7
                ...
                # shaped_reward[player_idx] += self.reward_shaping_params["PICKUP_TOMATO_REWARD"]
                shaped_reward[player_idx] += 0 # @GJX: Here

                if self.layout_name == 'far_tomato':
                    if USEFUL_TOMATO_PICKUP:
                        shaped_info[player_idx]["useful_tomato_pickup"] += 1
                        shaped_reward[player_idx] += self.reward_shaping_params["USEFUL_TOMATO_PICKUP"]
                    else:
                        shaped_info[player_idx]["useless_tomato_pickup"] += 1
                ...
                if self.is_dish_pickup_useful(new_state, pot_states):
                    ...
                    # shaped_info[player_idx][0] += self.reward_shaping_params["DISH_PICKUP_REWARD"]
            ...
            #         soup.begin_cooking()
                ...
                if player.get_object().name == 'dish' and self.soup_ready_at_location(new_state, i_pos):
                    ...
                    # shaped_info[player_idx][1] += self.reward_shaping_params["SOUP_PICKUP_REWARD"]
                    ...
                    if not soup.is_full:
                        ...
                        # shaped_info[player_idx][2] += self.reward_shaping_params["PLACEMENT_IN_POT_REW"]
        ...
        #     if new_state.put_first_tomato[player_idx]:
        #         sparse_reward[player_idx] = shaped_reward[player_idx] = 0
    ...
    def lossless_state_encoding(self, overcooked_state, horizon=400, debug=False):
        ...
        def process_for_player(primary_agent_idx):
            ...
            '''if primary_agent_idx == 0:
                print("process_for_player", primary_agent_idx)
