...
from environment.overcooked_new.Overcooked_Env import Overcooked
from environment.overcooked_new.src.overcooked_ai_py.mdp.actions import Action, Direction
from environment.overcooked_new.config import get_config
from environment.overcooked_new.src.overcooked_ai_py.mdp.overcooked_mdp import Recipe
import sys
import os
...
from environment.prompts.overcooked import GAME_STATE_PROMPT
...
class OvercookedEnv(BaseEnv):
    ...
    def observe(self):
        ...
        model2_description = GAME_STATE_PROMPT.format(
            ...
            my_orientation = orientation[1],
            my_holding = held_object[1],
            my_action_history = history[1],
            he_position = position[0],
            he_orientation = orientation[0],
            he_holding = held_object[0],
            he_action_history = history[0],
            kitchen_counter_state = text_kitchen_counter_state,
            pot_state = text_pot_state,
        )
        return {
    ...
    def summary(self):
        print(f"Seed {self.seed} - {self.all_args.layout_name} turn: {self.game_turn}, reward: {self.reward}")
